...
public class UpdateCurrentDriverProfileCommandHandler(
    IAppDbContext context,
    IUser currentUser) : IRequestHandler<UpdateCurrentDriverProfileCommand, Result<DriverCurrentProfileDto>>
{
    private readonly IAppDbContext _context = context;

    public async Task<Result<DriverCurrentProfileDto>> Handle(
        UpdateCurrentDriverProfileCommand request,
        CancellationToken ct)
    {
        ...
        var updateResult = user.UpdateProfile(request.Name, email: request.Email);
